...
import { submissionSchema, CANCER_TYPES, SCREENING_TYPES, SYMPTOMS, type SubmissionInput } from '@/lib/validators'
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <select className="input" aria-invalid={!!errors.sex} {...register('sex')}>
        ...
        <div>
          ...
          <select className="input" aria-invalid={!!errors.ageGroup} {...register('ageGroup')}>
            <option value="" hidden>선택</option>
          ...
          {errors.ageGroup && <p className="error">{errors.ageGroup.message}</p>}
        </div>

        <div>
          <label className="label">거주 지역 *</label>
          <select className="input" aria-invalid={!!errors.region} {...register('region')}>
            <option value="" hidden>선택</option>
        ...
        <div>
          ...
          <div className="mt-1 grid grid-cols-3 gap-2">
            <label className="inline-flex items-center gap-2">
              <input type="radio" value="NEVER" {...register('smokingStatus')} />
              비흡연
            </label>
            <label className="inline-flex items-center gap-2">
              <input type="radio" value="FORMER" {...register('smokingStatus')} />
              과거 흡연
            </label>
            <label className="inline-flex items-center gap-2">
              <input type="radio" value="CURRENT" {...register('smokingStatus')} />
              현재 흡연
            </label>
          </div>
          {errors.smokingStatus && <p className="error">{errors.smokingStatus.message}</p>}
        </div>
        ...
        <div>
          ...
          {errors.alcoholFrequency && <p className="error">{errors.alcoholFrequency.message}</p>}
        </div>
        ...
        <div>
          ...
          {errors.physicalActivity && <p className="error">{errors.physicalActivity.message}</p>}
        </div>

        <div>
          <label className="label">키 (cm)</label>
          <input className="input" type="number" min={100} max={250} placeholder="예: 170" {...register('heightCm')} />
        </div>

        <div>
          <label className="label">몸무게 (kg)</label>
          <input className="input" type="number" min={30} max={250} placeholder="예: 65" {...register('weightKg')} />
          {bmi && <p className="helper">예상 BMI: <b>{bmi}</b></p>}
        </div>
      </section>
          ...
          <div className="mt-1 grid grid-cols-2 md:grid-cols-3 gap-2">
            {CANCER_TYPES.map((t) => (
              <label key={t} className="inline-flex items-center gap-2">
                <input type="checkbox" value={t} {...register('personalHistory')} />
                {t}
              </label>
            ))}
          </div>
      ...
      <section className="space-y-4">
        <div>
          ...
          <label className="inline-flex items-center gap-2 mt-1">
            <input type="checkbox" {...register('screeningRecent')} />
            예 (해당 시 아래에서 검진 종류 선택)
          </label>
        </div>
        {screeningRecent && (
          <div>
            ...
            <div className="mt-1 grid grid-cols-1 md:grid-cols-2 gap-2">
              {SCREENING_TYPES.map((t) => (
                <label key={t} className="inline-flex items-center gap-2">
                  <input type="checkbox" value={t} {...register('screeningTypes')} />
                  {t}
                </label>
              ))}
            </div>
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            {errors.knowledgeLevel && <p className="error">{errors.knowledgeLevel.message}</p>}
          </div>
          <div>
            <label className="label">본인 암 발생 위험 체감 (1~5)</label>
            <input className="w-full" type="range" min={1} max={5} step={1} {...register('riskPerception')} />
      ...
      <section className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <select className="input" {...register('heardFrom')}>
              <option value="" hidden>선택(선택항목)</option>
          ...
          <div>
            ...
            <input className="input" type="email" placeholder="결과 안내를 원하시면 입력" disabled={!consentFollowup} {...register('email')} />
        ...
        <div>
          ...
          <textarea className="input h-28" placeholder="전하고 싶은 말씀을 자유롭게 남겨 주세요." {...register('comments')} />
      ...
      <section className="space-y-3">
        <label className="inline-flex items-start gap-2">
          ...
          <span>
            <b>참여 및 개인정보 처리에 동의</b>합니다. 본 설문은 익명으로 진행되며, 의료적 조언을 제공하지 않습니다.
            개인정보 안내를 <a href="/privacy">여기</a>에서 확인했습니다.
          </span>
        </label>
        {errors.consent && <p className="error">{errors.consent.message}</p>}
        <div className="flex items-center gap-3">
          <button className="btn" type="submit" disabled={isSubmitting}>제출</button>
          <span className="text-xs text-slate-400">제출을 누르면 위 내용에 동의한 것으로 간주됩니다.</span>
        </div>
      </section>
    </form>
  )
